...
public interface CargoRepository extends JpaRepository<Cargo, Integer> {
	...
	@Query("SELECT new kovalchuk.model.view.CargoView(c.id, g.name, c.weight, c.height, c.width, c.length, cFrom.name, cTo.name, c.price) FROM Cargo c JOIN c.goods g JOIN c.cityFrom cFrom JOIN c.cityTo cTo")
